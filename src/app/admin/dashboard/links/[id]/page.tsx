      ...
      <main className="px-6 pb-10 max-w-7xl mx-auto">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
            <div>
              ...
              <div className="flex items-center gap-2">
                ...
                <button onClick={() => { navigator.clipboard.writeText((data.domain ? "https://" + data.domain : process.env.NEXT_PUBLIC_BASE_URL) + "/s/" + data.shortCode); }} className="neu-btn p-1" title="Copy link">
            ...
            <div className="flex items-center gap-6">
              ...
              <div className="text-center"><p className="text-xs text-[var(--text-secondary)]">Owner</p><div className="flex items-center justify-center gap-2 mt-1"><Avatar email={data.userEmail || ""} size={24} tooltip /><p className="text-sm font-bold text-[var(--text)]">{data.userEmail || "Guest"}</p></div></div>
            </div>
          </div>
          <p className="text-sm text-[var(--text-secondary)] truncate">Destination: {data.originalUrl}</p>
        </div>
        ...
        {activeTab === "analytics" && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 animate-fade-in">
            <div className="neu-card !p-6"><h3 className="text-sm font-semibold text-[var(--text-secondary)] mb-4">Browsers</h3>{Object.entries(browsers).sort(([,a],[,b]) => b-a).map(([n,c]) => <div key={n} className="flex justify-between py-2 border-t border-[var(--shadow-dark)]/20 first:border-0"><span className="text-sm">{n}</span><span className="neu-badge text-xs">{c}</span></div>)}</div>
            <div className="neu-card !p-6"><h3 className="text-sm font-semibold text-[var(--text-secondary)] mb-4">Countries</h3>{Object.entries(countries).sort(([,a],[,b]) => b-a).map(([n,c]) => <div key={n} className="flex justify-between py-2 border-t border-[var(--shadow-dark)]/20 first:border-0"><span className="text-sm">{n}</span><span className="neu-badge text-xs">{c}</span></div>)}</div>
            <div className="neu-card !p-6"><h3 className="text-sm font-semibold text-[var(--text-secondary)] mb-4">Devices</h3>{Object.entries(devices).sort(([,a],[,b]) => b-a).map(([n,c]) => <div key={n} className="flex justify-between py-2 border-t border-[var(--shadow-dark)]/20 first:border-0"><span className="text-sm capitalize">{n}</span><span className="neu-badge text-xs">{c}</span></div>)}</div>
          </div>
        )}
      </main>
    </div>
